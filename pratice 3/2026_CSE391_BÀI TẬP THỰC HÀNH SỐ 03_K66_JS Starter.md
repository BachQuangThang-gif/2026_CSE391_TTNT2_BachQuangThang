...
- 1.Em thấy dòng Hello from JavaScript!
...
- Dòng log hiển thị Xin chào, mình là Thang, năm nay mình 20 tuổi.

  > - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?

- Nếu em quên dấu `+` thì code sẽ không chạy được và báo lỗi trên console là :[NEW] Explain Console errors by using Copilot in Edge: click
  to explain an error.
...
- Điều thú vị em vừa khám phá được về console là thay vì cứ quay lại file HTML hay CSS rồi ấn Save, thay vào đó ta thể sửa trực tiếp các thuộc tính ngay trên tab Console của Chrome.
- Em gặp lỗi ở cú pháp hay quên dấu `;`.Em đã xử lí bằng cách hỏi bạn , và tìm google
...
- Khi trường hợp `score >=8 ` là in ra `Giỏi`
- Khi trường hợp `score <5 ` là in ra `Yếu`
- Theo em cấu trúc `if/else` là vòng lặp của câu lệnh
...
>   - Thân hàm (body):Là toàn bộ phần code nằm trong cặp ngoặc nhọn { }. Cụ thể là các câu lệnh if/else để kiểm tra giá trị của avg và quyết định kết quả trả về.
>   - Giá trị trả về (return): Các chuỗi văn bản (string) như "Giỏi", "Khá", "Trung bình", hoặc "Yếu".
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
>   ---Tái sử dụng , dễ bảo trì , code sạch sẽ
...
>   -- Em thấy logic của if/else khiến em khó hiểu hoặc các tham số avg.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
>   --- Em đã hỏi bạn bè để vượt qua
...
>   Dòng lệnh này đóng vai trò như một "người đi tìm đồ".
> - Sự kiện `"click"` xảy ra khi nào?
>   Sự kiện "click" xảy ra ngay khoảnh khắc người dùng nhấn và nhả chuột (hoặc chạm tay trên màn hình cảm ứng) vào phần tử được gắn bộ lắng nghe (ở đây là nút btnHello).
...
>   Em có thể thay đổi qua màu khác
...
---Dom như một cái sơ đồ cây, nơi mà mỗi thẻ HTML
